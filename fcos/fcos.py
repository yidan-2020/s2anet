...
from dynamic_conv import DynamicBottleneck, DynamicConv2D, DynamicScale, get_module_running_cost
...
class FCOS(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            print(soft_cost / full_cost, used_cost / full_cost)
    ...
    def losses(self, gt_classes, gt_shifts_deltas, gt_centerness,
        ...
        storage.put_scalar("cost", (used_cost / full_cost).mean())
...
class FCOSHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        num_groups = cfg.MODEL.FCOS.NUM_GROUPS
        gate_activation = cfg.MODEL.FCOS.GATE_ACTIVATION
        gate_activation_kargs = cfg.MODEL.FCOS.GATE_ACTIVATION_KARGS
        resize_method = cfg.MODEL.FCOS.RESIZE_METHOD
        # fmt: on
        cls_subnet = []
        bbox_subnet = []
        for _ in range(num_convs // 2):
            cls_subnet_conv = DynamicBottleneck(
                in_channels,
                in_channels,
                kernel_size=3,
                stride=1,
